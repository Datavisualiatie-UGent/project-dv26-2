...
// Render the initial Vega-Lite visualization into the #vis div
const viewPromise = vegaEmbed("#vis-world-map", spec, { actions: false }).then(res => res.view);

// Initialize the Scrollama scroller
const scroller = scrollama();

const stepColors = {
  "1949": "#118ACB", // NATO light blue
  "1982": "#2E8B57", // Sea Green
  "1999": "#DAA520", // Goldenrod
  "2004": "#8A2BE2", // Blue Violet
  "2024": "#DC143C"  // Crimson
};
...
scroller
  ...
  .onStepEnter(async (response) => {
    // response contains { element, index, direction }
    const year = response.element.dataset.step;
    const color = stepColors[year] || "#118ACB";
    const view = await viewPromise;
    view.signal("natoColor", color).run();
  });
```
